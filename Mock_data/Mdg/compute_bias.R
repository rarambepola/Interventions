...
K_obs_obs <- K * (t_t_ind * beta00^2 + nt_nt_ind * beta01^2 + t_nt_ind * rho + nt_t_ind * rho)
K_cf_cf <- K * (t_t_ind * beta01^2 + nt_nt_ind * beta00^2 + t_nt_ind * rho + nt_t_ind * rho)
...
dist_sum <- 0
for(i in 1:n_obs){
  K_12_R <- K_obs_obs[-i, i]
  Y_fit_R[i] <- K_12_R %*% solve(K_obs_obs[-i, -i] + sigma_obs[-i, -i], Y[-i])
